...
//volatile double timestamp_double;
...
void SM_OS_Init(void)
{
  ...
  /* SPI read Thread*/
  osThreadDef(SPI_THREAD, spi_Thread, SPI_RD_THREAD_PRIO, 1, 300/4);
  /* SPI read Thread*/
  osThreadDef(I2C_THREAD, i2c_Thread, I2C_RD_THREAD_PRIO, 1, 300/4);
...
//	uint32_t isr_mask;
  ...
  /* DEBUG ONLY */
//  timestamp_double = timestamp_seconds;
  /* ---------- */
  
	taskEXIT_CRITICAL();
	/* -------- End critical section -----------*/
  return timestamp_seconds;
//	return ((double)timestamp/(double)SystemCoreClock);
}
...
uint8_t SM_StartSensorThread(uint8_t sId)
{
  ...
  else if(sId == iis2mdc_com_id)
  {
    IIS2MDC_Start();
  }
  else if(sId == imp34dt05_com_id)
  {
    IMP34DT05_Start();
  }
  else if(sId == mp23abs1_com_id)
  {
    MP23ABS1_Start();
  }
  ...
  else if(sId == lps22hh_com_id)
  {
    LPS22HH_Start();
  }
  else if(sId == stts751_com_id)
  {
    STTS751_Start();
  }  
  return 0;
