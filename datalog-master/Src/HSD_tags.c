...
const char HSD_HwTagPinNames[HSD_TAGS_MAX_HW_CLASSES][HSD_TAGS_LABEL_LENGTH] = { HSD_TAG_PIN0_NAME,
    HSD_TAG_PIN1_NAME,
    HSD_TAG_PIN2_NAME,
    HSD_TAG_PIN3_NAME,
    HSD_TAG_PIN4_NAME };
...
const uint16_t  TAG_PIN[HSD_TAGS_MAX_HW_CLASSES] =  { STMOD_PIN_7_PIN,
    STMOD_PIN_8_PIN,
    STMOD_PIN_9_PIN,
    STMOD_PIN_10_PIN,
    STMOD_PIN_11_PIN
...
GPIO_TypeDef  *TAG_PIN_PORT[HSD_TAGS_MAX_HW_CLASSES] = { STMOD_PIN_7_GPIO_PORT,
...
uint8_t TAG_Pin_Status[HSD_TAGS_MAX_HW_CLASSES] = {0xFF, 0xFF, 0xFF, 0xFF, 0xFF};
...
char *HSD_TAGS_get_tag_label(COM_Device_t *device, HSD_Tags_Type_t type, uint8_t class_id)
{
  ...
  if(type == HSD_TAGS_Type_Sw)
  {
    ...
    {
//      label = HSD_SwTagClasses[class_id];
  ...
  else if(type == HSD_TAGS_Type_Hw)
  {
    ...
    {
//      label = HSD_HwTagClasses[class_id];
...
int32_t HSD_TAGS_set_tag_label(COM_Device_t *device, HSD_Tags_Type_t type, uint8_t class_id, const char *label)
{
  ...
  if(type == HSD_TAGS_Type_Sw)
  {
    if(class_id < HSD_TAGS_MAX_SW_CLASSES)
    {
//      memcpy(HSD_SwTagClasses[class_id], label, label_len);
//      HSD_SwTagClasses[class_id][label_len] = '\0';
...
//      memcpy(device->tagList.HSD_HwTagClasses[class_id], label, label_len);
//      device->tagList.HSD_HwTagClasses[class_id][label_len] = '\0';
    }
    else
    {
      return -1;
